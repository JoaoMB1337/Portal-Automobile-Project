                ...
                <div>
                    <div class="flex items-center mb-4">
                        <div class="font-semibold text-center w-full">
                            ...
                            <a href="{{ route('employees.show', Auth::user()->id) }}" class="text-[#f84525] font-medium text-sm hover:text-red-700 mt-3">Ver Perfil</a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                ...
                @forelse ($activeTrips as $trip)
                    <tr>
                        <td class="py-4 whitespace-nowrap text-center text-gray-800 cursor-pointer" onclick="showTripDetails('{{ $trip->id }}', '{{ $trip->start_date }}', '{{ $trip->end_date }}', '{{ $trip->destination }}', '{{ implode(', ', $trip->vehicles->pluck('plate')->toArray()) }}', '{{ $trip->project->country->name }}', '{{ $trip->project->district->name ?? 'N/A' }}')">
                            {{ $trip->start_date }}
                        </td>
                        <td class="py-4 whitespace-nowrap text-center text-gray-800 cursor-pointer" onclick="showTripDetails('{{ $trip->id }}', '{{ $trip->start_date }}', '{{ $trip->end_date }}', '{{ $trip->destination }}', '{{ implode(', ', $trip->vehicles->pluck('plate')->toArray()) }}', '{{ $trip->project->country->name }}', '{{ $trip->project->district->name ?? 'N/A' }}')">
                            {{ $trip->destination }}
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium text-gray-500">Nenhuma viagem atribuída hoje.</td>
                    </tr>
                @endforelse
                </tbody>
