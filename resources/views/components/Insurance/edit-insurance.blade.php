        ...
        <div class="max-w-md w-full bg-white rounded-xl p-7 custom-card mt-12">
            ...
            <form method="POST" action="{{ route('insurances.update', $insurance->id) }}" class="space-y-6">
                ...
                <div>
                    ...
                    <input id="cost" type="text" class="form-input w-full rounded-md border-gray-300 focus:border-gray-400 focus:ring focus:ring-gray-200 @error('cost') border-red-500 @enderror" name="cost" value="{{ old('cost', $insurance->cost) }}" required>
                    @error('cost')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
