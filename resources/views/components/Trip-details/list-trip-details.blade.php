<div class="container">
    @if (Auth::check() && Auth::user()->isAdmin())
        <div class="form-container">
            ...
            <a href="{{ route('trip-details.index', ['clear_filters' => true]) }}"
                class="px-4 py-2 bg-gray-700 text-white rounded-md shadow-sm hover:bg-gray-800">Limpar</a>
        </div>
        ...
        <table>
            ...
            <tbody>
                @forelse ($tripDetails as $tripDetail)
                    @php
                        $employeeName =
                            $tripDetail->trip && $tripDetail->trip->employee
                                ? $tripDetail->trip->employee->name
                                : 'Funcionário não encontrado';
                        $tripName = $tripDetail->trip ? $tripDetail->trip->destination : 'Viagem não encontrada';
                        $costTypeName = $tripDetail->costType
                            ? $tripDetail->costType->type_name
                    ...
                    <tr>
                        ...
                        <td>
                            <!-- Ações -->
                            <a href="{{ route('trip-details.show', $tripDetail->id) }}"
                                class="text-indigo-600 hover:text-indigo-900">Detalhes</a>
                        </td>
                    </tr>
                @empty
                    <tr>
